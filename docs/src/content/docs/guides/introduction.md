...
Under the hood, supawho stores each token in your operating system's **native secret vault** — macOS Keychain, Linux Secret Service, or Windows Credential Manager — so tokens are encrypted and **never written to a file**. See [Security](/guides/security/) for details.

## Beyond switching

- [`supawho whoami`](/commands/whoami/) — map every saved account to its real email and organizations
- [`supawho find <ref>`](/commands/find/) — the reverse lookup: which account owns a given project?
- [`supawho upgrade`](/commands/upgrade/) — self-update, checksum-verified, on any OS

## Next steps

- [Getting started](/guides/getting-started/) — install and save your first account
- [Commands](/commands/) — the full reference, one page per command
- [How it works](/guides/how-it-works/) — the model behind supawho
